...
from utils import set_seed, get_dataset, get_model, SupConResNet, LinearClassifier
...
supcon = SupConResNet(base_model).to(device)
classifier = LinearClassifier().to(device)

loaded_info_sup = torch.load(f'./weights/mpl_supcon_encoder.tar')
supcon.load_state_dict(loaded_info_sup[f'model_state_dict'])
print('supcon state loaded!!')

loaded_info_clf = torch.load(f'./weights/mpl_supcon_classifier.tar')
classifier.load_state_dict(loaded_info_clf[f'model_state_dict'])
...
def evaluate(model, classifier, criterion, eval_iter):
  ...
  features = model.encoder(aug_img)
...
for epoch in range(args.epochs):
  ...
  evaluate_acc, evaluate_loss, evaluate_roc = evaluate(supcon, classifier, criterion, eval_iter)
